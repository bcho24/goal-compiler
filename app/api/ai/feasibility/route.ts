...
import { buildFeasibilityPrompt } from '@/lib/ai/prompts';
import type { CompatType } from '@/lib/types';

export async function POST(req: NextRequest) {
  try {
    const { goalText, clarifications, compatType, baseURL, apiKey, model } = await req.json();

    if (!apiKey) {
      return NextResponse.json({ error: 'API key not configured / API Key 未配置' }, { status: 400 });
    }

    const prompt = buildFeasibilityPrompt(goalText, clarifications || []);
    ...
    console.error('[/api/ai/feasibility] Error:', message);
